...
- 服務/部署：`serve/`(FastAPI 推論 API)、`monitoring/`(Prometheus+Grafana，podman pod)、
  `registry/`(模型治理台帳+model card，**進 git** 當審計軌跡)、`Containerfile`(GPU via CDI)。
- 模型身份用 ckpt 的 sha256 digest（`src/registry.py`），上 production 要過 promotion gate。
